...
router = APIRouter(tags=["assets-alerts"], dependencies=[Depends(get_current_user)])
...
def get_asset_endpoint(asset_id: int, db: Session = Depends(get_db)):
    return get_asset(db, asset_id)


@router.put("/assets/{asset_id}", response_model=AssetRead)
def update_asset_endpoint(asset_id: int, payload: AssetUpdate, db: Session = Depends(get_db)):
...
def update_alert_endpoint(alert_id: int, payload: AlertUpdate, db: Session = Depends(get_db)):
